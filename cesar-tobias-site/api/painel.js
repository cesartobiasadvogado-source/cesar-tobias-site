...
module.exports = async (req, res) => {
  ...
  if (acao === 'executar') {
    ...
    var camposPermitidos = ['tipo', 'nome', 'tipo_servico', 'descricao', 'endereco', 'etapa', 'origem', 'midia', 'campanha', 'resposta', 'valor', 'vencimento', 'entrada', 'parcelas', 'data_entrada', 'data_pagamento', 'dia_vencimento', 'percentual_honorarios', 'percentual_recursal', 'campos_extra', 'linha_contrato', 'numero_parcela', 'prazo_dias', 'clausula_honorarios', 'descricao_debito', 'pdf_id', 'email', 'whatsapp', 'valor_total', 'valor_entrada', 'forma_pagamento', 'num_parcelas', 'data_inicio', 'data_primeira_parcela', 'mes', 'percentual_exito', 'valor_recebido_cliente', 'status', 'tipo_contrato', 'processo_numero', 'periodicidade', 'id', 'tipo_registro', 'usar_asaas', 'tipo_despesa', 'data_despesa', 'reembolsavel', 'categoria', 'observacoes', 'data_termino', 'telefone', 'ids', 'nomes', 'meses', 'valor_causa', 'valor_ganho_causa', 'valor_estimado_ganho', 'valor_ja_recebido'];
  ...
  if (acao === 'audiencias') {
    ...
    if (opAud === 'atualizar_tags') {
      if (corpo.id) urlAud += '&id=' + encodeURIComponent(corpo.id);
      var tagsTexto = Array.isArray(corpo.tags) ? corpo.tags.join(',') : (corpo.tags || '');
      urlAud += '&tags=' + encodeURIComponent(tagsTexto);
    }
    if (opAud === 'comparar' && corpo.id) {
      urlAud += '&id=' + encodeURIComponent(corpo.id);
    }
    if (opAud === 'gerar_email' && corpo.id) {
      urlAud += '&id=' + encodeURIComponent(corpo.id);
    }
